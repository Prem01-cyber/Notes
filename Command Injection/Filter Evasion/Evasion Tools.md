...
```shell-session
premjampuram@htb[/htb]$ git clone https://github.com/Bashfuscator/Bashfuscator
premjampuram@htb[/htb]$ cd Bashfuscator
premjampuram@htb[/htb]$ python3 setup.py install --user
```
...
```shell-session
premjampuram@htb[/htb]$ cd ./bashfuscator/bin/
premjampuram@htb[/htb]$ ./bashfuscator -h
...
```shell-session
premjampuram@htb[/htb]$ ./bashfuscator -c 'cat /etc/passwd'
...
```shell-session
premjampuram@htb[/htb]$ ./bashfuscator -c 'cat /etc/passwd' -s 1 -t 1 --no-mangling --layers 1
...
```shell-session
premjampuram@htb[/htb]$ bash -c 'eval "$(W0=(w \  t e c p s a \/ d);for Ll in 4 7 2 1 8 3 2 4 8 5 7 6 6 0 9;{ printf %s "${W0[$Ll]}";};)"'
