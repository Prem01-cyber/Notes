...
###### `Created on :- 2023-06-23 - 21:33`
###### `Created by:- Prem J`
***

- In some cases there might be protections deployed on target website which may cause some errors, we can manage to bypass the protection and perform automated exploitation.

#### `Anti CSRF Token Bypass -->`

- One of the basic defences against the usage of automation tools is the incorporation of anti-CSRF tokens into HTTP requests.
...
- Each token contains a unique, unpredictable, secret value that is not guessable by a third party.
- If the relevant token is not included, the server-side application rejects the request. So, only original users are authenticated to send requests in a given session.
- Without adding CSRF tokens the web application maybe prone to [[XSS Discovery]]

- SQLMap has options that can help in bypassing anti-CSRF protection i.e `--csrf-token`. By specifying token parameter name (which should already be in the request data). SQLMap will automatically start parsing response content and search for fresh token values.

>[!Note]
>even in a case where the user does not explicitly specify the token's name via `--csrf-token`, if one of the provided parameters contains any of the common infixes (i.e. `csrf`, `xsrf`, `token`), the user will be prompted whether to update it in further requests

```shell-session
prem@htb[/htb]$ sqlmap -u "http://www.example.com/" --data="id=1&csrf-token=WfF1szMUHhiokx9AHFply5L2xAOfjRkE" --csrf-token="csrf-token"
...
- In some cases, the web application may only require unique values to be provided inside predefined parameters. Such a mechanism is similar to the anti-CSRF technique described above, except that there is no need to parse the web page content
- So by simply ensuring that each request has a unique value for predefined parameter the web application can prevent CSRF  attempts.
- This method can be evaded through `--randomize` option in SQLMap

```shell-session
prem@htb[/htb]$ sqlmap -u "http://www.example.com/?id=1&rp=29125" --randomize=rp --batch -v 5 | grep URI
...
- Another similar mechanism is where a web application expects a proper parameter value to be calculated based on some other parameter value(s)
...
```shell-session
prem@htb[/htb]$ sqlmap -u "http://www.example.com/?id=1&h=c4ca4238a0b923820dcc509a6f75849b" --eval="import hashlib; h=hashlib.md5(id).hexdigest()" --batch -v 5 | grep URI
